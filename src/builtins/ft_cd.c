...
/*   Updated: 2025/01/28 18:44:26 by liamcohen        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_or_create_pwd(t_environment_var *env)
{
	...
	ft_memset(current_dir, 0, PATH_MAX);
	if (!getcwd(current_dir, PATH_MAX))
		return (print_error_exec_message(CD_ERR_RETRIEVING_CURR_DIRECTORY,
				"cd"), ERROR);
	if (!is_env_var_exist("PWD", env))
	{
		if (create_env_var(&env, "PWD", current_dir) == ERROR)
			return (print_error_message("cd: error create PWD var"), ERROR);
	}
	else
	{
		if (update_env_var_value("PWD", current_dir, env) == ERROR)
			return (print_error_message("cd: error update PWD var"), ERROR);
	}
	return (TRUE);
}
...
int	update_or_create_oldpwd(t_environment_var *env)
{
	...
	{
		if (create_env_var(&env, "OLDPWD", pwd->value) == ERROR)
