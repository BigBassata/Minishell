...
/*   Updated: 2025/03/09 21:06:08 by licohen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	execute_exit_builtin(t_command *cmd, t_environment_var **env)
{
	int	exit_status;

	if (ft_strcmp(cmd->args[0], "exit") == 0)
	{
		exit_status = ft_exit(cmd, cmd->output_fd);
		if (exit_status >= 0)
		{
			close_all_open_fds();
...
int	execute_builtin(t_command *cmd, t_environment_var **env)
{
	...
	if (ft_strcmp(cmd->args[0], "echo") == 0)
		...
		return (execute_exit_builtin(cmd, env));
	(*env)->last_exit_code = cmd->exit_code;
	return (0);
}
